...
function mergeCustomTransformers(
  ...
  if (consumerCustomTransformers.before) {
    mergedCustomTransformers.before = originalCustomTransformers?.before
      ? [...originalCustomTransformers.before, ...consumerCustomTransformers.before]
  ...
  if (consumerCustomTransformers.after) {
    mergedCustomTransformers.after = originalCustomTransformers?.after
      ? [...originalCustomTransformers.after, ...consumerCustomTransformers.after]
      : consumerCustomTransformers.after;
  }

  if (consumerCustomTransformers.afterDeclarations) {
    mergedCustomTransformers.afterDeclarations = originalCustomTransformers?.afterDeclarations
      ? [...originalCustomTransformers.afterDeclarations, ...consumerCustomTransformers.afterDeclarations]
...
function getNormalizedTsConfig(options: TypeScriptCompilationOptions) {
  ...
  if (tsConfig.options.incremental && !tsConfig.options.tsBuildInfoFile) {
    tsConfig.options.tsBuildInfoFile = joinPathFragments(options.outputPath, 'tsconfig.tsbuildinfo');
  }
  return tsConfig;
}
...
function normalizeOptions(options: TypeScriptCompilationOptions): TypeScriptCompilationOptions {
  return {
    ...options,
