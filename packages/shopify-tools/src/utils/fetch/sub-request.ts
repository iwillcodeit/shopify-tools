...
import { CacheShort, type CachingStrategy, type AllCacheOptions } from './strategies.js';
 ...
 * @private
 */
export async function getItemFromCache(cache: Cache, key: string): Promise<undefined | [any, Response]> {
  if (!cache) return;
 ...
 * @private
 */
export async function setItemInCache(cache: Cache, key: string, value: any, userCacheOptions?: CachingStrategy) {
  if (!cache) return;
