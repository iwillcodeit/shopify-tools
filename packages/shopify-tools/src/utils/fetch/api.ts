import { CacheShort, generateCacheControlHeader, type CachingStrategy } from './strategies.js';
...
function getCacheControlSetting(userCacheOptions?: CachingStrategy, options?: CachingStrategy): CachingStrategy {
...
function generateDefaultCacheControlHeader(userCacheOptions?: CachingStrategy): string {
 ...
 */
async function getItem(cache: Cache, request: Request): Promise<Response | undefined> {
  if (!cache) return;
 ...
 */
async function setItem(cache: Cache, request: Request, response: Response, userCacheOptions: CachingStrategy) {
  if (!cache) return;
